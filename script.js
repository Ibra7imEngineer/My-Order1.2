...
function finishOrder() {
    ...
    try {
        ...
        console.log('✅ تم حفظ الطلب في السجل المركزي بنجاح - إجمالي الأوردرات:', allOrders.length);
    } catch (e) {
...
function renderAllOrders() {
    ...
    listContainer.innerHTML = orders.slice().reverse().map((order, idx) => {
...
function validateAdminLogin() {
    ...
    console.log('📝 البيانات المدخلة:', { username });
    ...
    console.log('📋 بيانات المدير المتوقعة:');
    console.log('   - اسم المستخدم:', ADMIN_ACCOUNT.username);
    console.log('   - كلمة المرور:', ADMIN_ACCOUNT.password);
    
    // التحقق من بيانات المدير
    if (username === ADMIN_ACCOUNT.username && password === ADMIN_ACCOUNT.password) {
        console.log('✅ ✅ ✅ تم التحقق من البيانات بنجاح! ✅ ✅ ✅');
        
        // بيانات صحيحة - حفظ بيانات الجلسة
        try {
        ...
        showNotification(`✨ أهلاً وسهلاً ${ADMIN_ACCOUNT.name}! تم تسجيل دخولك بنجاح 🎉`, 'success');
        
        // محاولة تسجيل النشاط
        try {
            if (typeof logCustomerActivity === 'function') {
                logCustomerActivity('admin', 'دخول إدارة', `دخول المدير ${ADMIN_ACCOUNT.username} إلى لوحة التحكم`);
                console.log('✅ تم تسجيل نشاط الدخول');
            }
        } catch (e) {
            console.warn('⚠️ تم الدخول لكن لم يتم تسجيل النشاط:', e.message);
        }
        
        // الانتقال إلى لوحة الإدارة
        console.log('📄 الانتقال إلى صفحة الإدارة بعد 500ms...');
        setTimeout(() => {
            console.log('🎯 استدعاء showPage(admin-page)');
            showPage('admin-page');
            console.log('✅ تم الانتقال إلى لوحة الإدارة');
        }, 500);
    } else {
        // بيانات غير صحيحة
        console.error('❌ ❌ ❌ بيانات دخول خاطئة ❌ ❌ ❌');
        console.log('   - اسم المستخدم المدخل:', username);
        console.log('   - اسم المستخدم المتوقع:', ADMIN_ACCOUNT.username);
        console.log('   - مطابقة الاسم:', username === ADMIN_ACCOUNT.username);
        console.log('   - مطابقة كلمة المرور:', password === ADMIN_ACCOUNT.password);
...
let customersActivityLog = JSON.parse(localStorage.getItem('customersActivityLog')) || [];
...
function logCustomerActivity(customerId, activityType, description) {
    ...
    localStorage.setItem('customersActivityLog', JSON.stringify(customersActivityLog));
...
function renderSalesLog() {
    ...
    // عرض كل شهر وعملاؤه
    const monthlyReportsHTML = sortedMonths.map((month, idx) => {
        const entries = monthlyData[month];
        const entriesCount = entries.length;
        
        const entriesTableHTML = entries.map((entry, entryIdx) => {
            const date = new Date(entry.registeredAt);
...
function printMonthlyReport() {
    ...
    Object.keys(monthlyData).reverse().forEach((month, idx) => {
        const entries = monthlyData[month];
        htmlContent += `
...
function clearSalesLog() {
    if (confirm('⚠️ هل أنت متأكد من حذف سجل المبيعات بالكامل؟ لا يمكن التراجع عن هذا الإجراء')) {
...
function switchAdminTab(tabName) {
    ...
    const activitiesContent = document.getElementById('adminTabActivitiesContent');
    ...
    [itemsTab, ordersTab, salesTab, customersTab, activitiesTab].forEach(tab => {
        if (tab) {
    ...
    if (tabName === 'items') {
        ...
        if (customersTab) {
            customersTab.style.background = 'linear-gradient(135deg, #9B59B6, #8E44AD)';
        ...
        if (activitiesTab) {
            activitiesTab.style.background = 'linear-gradient(135deg, #E74C3C, #C0392B)';
...
function renderCustomersActivityLog() {
    ...
    const registrations = customersActivityLog.filter(a => a.activityType === 'تسجيل جديد').length;
    const logins = customersActivityLog.filter(a => a.activityType === 'دخول').length;
    const logouts = customersActivityLog.filter(a => a.activityType === 'خروج').length;
        ...
        <div style="background:white; border-radius:14px; padding:25px; box-shadow:0 4px 15px rgba(0,0,0,0.08); overflow-x:auto;">
            <table id="activityTable" style="width:100%; border-collapse:collapse;">
                ...
                <tbody id="activityTableBody">
                    ${customersActivityLog.slice().reverse().map((activity, idx) => {
                        ...
                        const activityTypeColor = 
                            activity.activityType === 'تسجيل جديد' ? '#27AE60' :
                            activity.activityType === 'دخول' ? '#3498DB' :
                            activity.activityType === 'خروج' ? '#F39C12' : '#9B59B6';
...
function filterActivityLog() {
    ...
    rows.forEach(row => {
        ...
        if (searchText) {
            ...
            show = show && (customerName.includes(searchText) || email.includes(searchText));
        }
        ...
        if (selectedDate && show) {
            ...
            const selectedDateObj = new Date(selectedDate).toLocaleDateString('ar-EG');
            // استخلاص التاريخ من الوقت المعروض
            show = show && rowDate.includes(new Date(selectedDate).toLocaleDateString('ar-EG'));
        }
...
function exportActivityLogToCSV() {
    ...
    let csv = 'الرقم,اسم العميل,البريد الإلكتروني,نوع النشاط,التاريخ,الوقت,التفاصيل\n';
    
    customersActivityLog.slice().reverse().forEach((activity, idx) => {
        const customer = customers.find(c => c.id === activity.customerId);
            ...
            <table>
                ...
                <tbody>
                    ${customersActivityLog.slice().reverse().map((activity, idx) => {
                        ...
                        if (activity.activityType === 'دخول') typeClass = 'type-login';
                        if (activity.activityType === 'خروج') typeClass = 'type-logout';
...
function customerLogin() {
    ...
    if (customers.length === 0) {
        errorDiv.textContent = '❌ لا توجد حسابات مسجلة في النظام. الرجاء إنشاء حساب جديد أولاً';
        errorDiv.style.display = 'block';
    ...
    console.log('🔍 نتيجة البحث:', customer ? 'عميل وجد' : 'لم يتم العثور على عميل');
    ...
    if (customer) {
        ...
        console.log('📋 قائمة رسائل البريد المسجلة:', customers.map(c => c.email));
    }
}
...
function showEditEmailDialog() {
    ...
    document.getElementById('confirmEmailBtn').onclick = () => {
        ...
        let customers = JSON.parse(localStorage.getItem('customers')) || [];
        const emailExists = customers.some(c => c.email === newEmail && c.id !== currentCustomer.id);
